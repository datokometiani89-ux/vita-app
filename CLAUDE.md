...
- ✅ **Wearable / health-data seam (v60)** `#/wearable` (`V.screens.wearable`) — connect Apple Health / Google Fit / Garmin / Fitbit; **demo import** (`V.connectWearable` generates a plausible snapshot + feeds `wellness.hr`/`wellness.sleep` so readiness/bio-age use it). `state.wearable`; `V.WEARABLES`. NB: real sync needs native HealthKit / Google Fit OAuth — labelled demo. Menu tile `mWearable`. i18n `we*`.
- **NEXT:** in-app telemedicine (video + e-prescription + payment, Doctolib model) on the clinics-booking seam · B2B2C.
...
rsync to `/tmp/vita-preview`, bump `?v=NN` (app.html) + `sw.js` CACHE, reload with `?cb=`, drive the UI, check `preview_console_logs`. Currently at **v60**. Launch config `.claude/launch.json` runs `python3 http.server` on the mirror, port 8011 (note: it `chdir`s first — the launched cwd is sandboxed/inaccessible, so `--directory` fails).
