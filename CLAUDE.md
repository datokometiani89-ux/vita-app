...
- ✅ **Tongue modality — 5th system (v78–v82)** `#/tonguescan` (`V.screens.tonguescan`). Inspired by published tongue-image diagnostics (Middle Tech U / UniSA 2024; "Dr. Tongue" sign-oriented detection) but framed as **observable signals, not diagnosis**. **`V.tongueAnalyze(data,w,h)`** is the single pure entry point — an analytical on-device pipeline: **segmentation** (mask reddish tongue body + whitish coating film via `tgIsTongue`/`tgIsBody`, central-crop, bbox; body colour read from red pixels only so coating doesn't wash it out), **quality gating** (too-dark/overexposed/no-tongue/blurry → `tgQ*` retake message) + **confidence** (coverage+contrast+lighting), and signals: colour band (`tgClassifyColor` pale/normal/red/purple), coating %, cracks (dark fissures), teeth-marks (boundary scalloping), moisture (specular gloss), hue uniformity. **Zone-by-zone readout** splits root/centre/tip, classifies each (colour + per-zone coating %), renders a tinted SVG tongue + organ-zone note (tip=heart/lungs, centre=stomach, root=kidney). **Trend** card = colored band dots over time. Optional **Phase-2 AI reading**: opt-in button → `V.api.vision(dataUrl, mime, "tongue")` (serve.py `handle_vision` `kind` param; non-diagnostic prompt → colour/coating/signs/note; busy/429 → retry, else silent fallback to on-device). Wired into `#/scan` modality strip + full-body scan as the **5th system** (composite, body-map mouth dot, legend, step, AI-report summary). `state.wellness.tongueScan`. Kept low-level `V.tongueColor/Coating/Surface/Flag` (unused by the screen now). i18n `tg*`; CSS `.tg-*`. NB watch-out: **zone() bounds must be integer** — float loop index makes `mask[y*w+x]` undefined (centre/tip read empty); `Math.floor` the zone y-bounds.
...
rsync to `/tmp/vita-preview`, bump `?v=NN` (app.html) + `sw.js` CACHE, reload with `?cb=`, drive the UI, check `preview_console_logs`. Currently at **v82**. NB: the browser/SW caches `js/*.js?v=NN`, so a `?cb=` on app.html alone can still serve a **stale script** after edits at the same version — bump `NN` (and/or unregister SW + clear caches) when verifying mid-version edits. Launch config `.claude/launch.json` runs `python3 http.server` on the mirror, port 8011 (note: it `chdir`s first — the launched cwd is sandboxed/inaccessible, so `--directory` fails).
